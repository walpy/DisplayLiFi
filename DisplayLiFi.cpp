...
DisplayLiFi::DisplayLiFi() {
}

void DisplayLiFi::setLightReciever(LightReciever *lightReciever) {
  lightReciever_ = lightReciever;
}
...
void Protocol::pushRGB(uint8_t rgb) {
  if (rgb == START_BIT) {
    buffer_ = 1;
    return;
  }
  ...
  buffer_ |= (rgb >> 2) & 1;
  if (buffer_ & 0x80) {
    uint8_t parityExpected = (rgb >> 1) & 1;
